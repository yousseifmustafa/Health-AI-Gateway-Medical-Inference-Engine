...
async def async_Post_Query_Validate(Model, user_query: str, answer: str) -> str:
    """
    ...
    try:
        raw_response_content = await asyncio.to_thread(Model, final_prompt)
        ...
        if not rewritten_answer or len(rewritten_answer) < 10:
            ...
            print("INFO: Answer successfully rewritten and validated (async).")
            return rewritten_answer

    except Exception as e:
        print(f"CRITICAL: Error during async answer rewriting process: {e}. Returning original answer.")
        return answer